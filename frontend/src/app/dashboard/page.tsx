...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (lastMessage?.type === 'SCRAPING_STARTED' && lastMessage.data) {
        const { competitor_name } = lastMessage.data;
        setScanningCompetitor(competitor_name || 'Competitor');
    } else if (lastMessage?.type === 'ANALYSIS_COMPLETE' && lastMessage.data) {
        setScanningCompetitor(null);
        toast.success('Scan finished. New information available.');
    }
  }, [lastMessage]);
  ...
  return (
    <div className="space-y-8">
      ...
      {scanningCompetitor ? (
        <TaskProgress competitorName={scanningCompetitor} />
      ) : (
        <div className="grid gap-6 md:grid-cols-3">
          <StatCard
